...
DB_PATH = "/tmp/smartcaixilho_db.json"
...
# ── Alerta de e-mail via SendGrid ─────────────────────────────────────────────
def enviar_alerta_email(empresa, responsavel, total, pct, nivel):
    try:
        api_key     = st.secrets.get("SENDGRID_API_KEY", "")
        email_dest  = st.secrets.get("EMAIL_DESTINO", "")
        if not api_key or not email_dest:
            return False

        assunto = f"[Smart Caixilho] Nova empresa: {empresa}"
        corpo   = f"""
...
<table style="border-collapse:collapse;font-family:Arial,sans-serif">
  ...
  <tr><td style="padding:8px;font-weight:bold">Responsável</td><td style="padding:8px">{responsavel}</td></tr>
  <tr><td style="padding:8px;font-weight:bold">Pontuação</td><td style="padding:8px">{total}/40 ({pct}%)</td></tr>
  <tr><td style="padding:8px;font-weight:bold">Nível</td><td style="padding:8px">{nivel}</td></tr>
</table>
<br>
<p>⬇️ Acesse o painel do orientador para baixar o CSV/JSON atualizado.</p>
<p style="color:#888;font-size:12px">Smart Caixilho — ASES Consultoria em Esquadrias</p>
"""
        payload = {
            "personalizations": [{"to": [{"email": email_dest}]}],
            "from":             {"email": email_dest, "name": "Smart Caixilho"},
            "subject":          assunto,
...
if st.button("📊 FINALIZAR DIAGNÓSTICO E GERAR RELATÓRIO"):
    if not empresa or not email:
        ...
        enviar_alerta_email(empresa, responsavel, total, int(pct), nivel)
...
if senha == "cba2026":
    ...
    # ── Teste de e-mail ───────────────────────────────────────────────────────
    st.markdown("#### 📧 Teste de E-mail")
    if st.button("Enviar e-mail de teste"):
        api_key    = st.secrets.get("SENDGRID_API_KEY", "")
        email_dest = st.secrets.get("EMAIL_DESTINO", "")
        st.write(f"API Key encontrada: {'✅ Sim' if api_key else '❌ Não'}")
        st.write(f"E-mail destino: {email_dest if email_dest else '❌ Não encontrado'}")
        if api_key and email_dest:
            resultado = enviar_alerta_email("Empresa Teste", "Responsável Teste", 25, 62, "Nível 3 – Operação Controlada")
            if resultado:
                st.success("✅ E-mail enviado com sucesso!")
            else:
                # Tentar envio direto para ver erro detalhado
                try:
                    payload = {
                        "personalizations": [{"to": [{"email": email_dest}]}],
                        "from":             {"email": email_dest, "name": "Smart Caixilho"},
                        "subject":          "Teste Smart Caixilho",
                        "content":          [{"type": "text/html", "value": "<p>Teste de e-mail.</p>"}]
                    }
                    data = json.dumps(payload).encode("utf-8")
                    req  = urllib.request.Request(
                        "https://api.sendgrid.com/v3/mail/send",
                    ...
                    try:
                        with urllib.request.urlopen(req) as resp:
                            st.success(f"✅ Status: {resp.status}")
                    except urllib.error.HTTPError as http_err:
                        erro_body = http_err.read().decode("utf-8")
                        st.error(f"❌ HTTP {http_err.code}: {erro_body}")
                except Exception as ex:
                    st.error(f"❌ Erro: {ex}")
        else:
            st.error("❌ API Key ou e-mail não encontrados nos Secrets.")
    ...
    if dados:
        ...
        st.download_button(
            ...
            "SmartCaixilho_Resultados.csv",
            "text/csv"
        )
        st.download_button(
            "📥 Baixar Base Completa (JSON)",
            json.dumps(dados, ensure_ascii=False, indent=2).encode("utf-8"),
            "SmartCaixilho_Resultados.json",
            "application/json"
        )
    else:
